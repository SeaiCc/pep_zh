...
specified as keyword arguments.Note: implementations are allowed to implement a subset of the full
...
that will happen when the connection handshake is complete.(*) There is no requirement that protocol_factory is a class.
...
DatagramProtocol. These are described later.Mandatory positional argument:
...
exceptions uniformly by third-party libraries.The loop uses default_exception_handler() if the default was not
...
- The loop checks for slow callbacks to detect accidental blocking for I/O.The loop.slow_callback_duration attribute controls the maximum
...
t.write(b'abcdef'), as well as to:t.write(b'a')
t.write(b'b')
t.write(b'c')
t.write(b'd')
t.write(b'e')
...
writelines(iterable). Equivalent to:for data in iterable:
 self.write(data)
...
low-water limits for flow control.These two values control when to call the protocol’s
...
treated exactly equivalent to:p.data_received(b'abc')
...
closed as soon as a “closure alert” is received.The default implementation returns None.
...
when waited for. Example of use:for f in as_completed(fs):
...
Future is cancelled, the argument Future is unaffected.A use case for this function would be a coroutine that caches a
...
intended use is like this:writer.write(data)
...
- Copyright
 ...
 [Page Source (GitHub)](https://github.com/python/peps/blob/main/peps/pep-3156.rst?plain=1)
