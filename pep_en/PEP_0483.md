...
this type, since one can call len on them:len([1, 2, 3]) # OK
len('abc') # also OK
len(42) # not a member of Sized

By a simple class definition, for example if one defines a class:class UserID(int):
 pass
...
- Union[str, int] is a type but not a proper class:class MyUnion(Union[str, int]): ... # raises TypeError
...
them as such:users = [] # type: List[UserID]
...
Function type annotation with a constrained type variable:AnyStr = TypeVar('AnyStr', str, bytes)
...
subclass would be chosen as the return type, e.g.:S = TypeVar('S')
...
- Type aliases, e.g.:Point = Tuple[float, float]
def distance(point: Point) -> float: ...

Forward references via strings, e.g.:class MyComparable:
...
arguments, thus avoiding any special syntax, e.g.:T = TypeVar('T', bound=complex)
...
Type declaration in comments, e.g.:lst = [] # type: Sequence[int]

Casts using cast(T, obj), e.g.:zork = cast(Any, frobozz())
...
- Copyright
 ...
 [Page Source (GitHub)](https://github.com/python/peps/blob/main/peps/pep-0483.rst?plain=1)
