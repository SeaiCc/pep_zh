...
The public C API should be compatible with C99 and C++.(As a reminder to any users reading this: this PEP is a style guide; these
...
code requires braces. Braces should be formatted as shown:if (mro != NULL) {
 ...
}
else {
 ...
...
The return statement should not get redundant parentheses:return(albatross); /* incorrect */
...
e.g.:PyErr_Format(PyExc_TypeError,
 "cannot create '%.100s' instances",
 type->tp_name);

When you break a long expression at a binary operator, braces
should be formatted as shown:if (type->tp_dictoffset != 0
...
(See PEP 8 for a longer discussion.)
...
Example:#define ADD_INT_MACRO(MOD, INT) \
 do { \
...
the PyAPI_FUNC() macro and PyAPI_DATA() macro, like this:PyAPI_FUNC(PyObject *) PyObject_Repr(PyObject *);
...
For example:PyDoc_STRVAR(myfunction__doc__,
...
concatenation:PyDoc_STRVAR(myfunction__doc__,
"myfunction(name, value) -> bool\n\n"
"Determine whether name and value make a valid pair.");
...
- Copyright
 ...
 [Page Source (GitHub)](https://github.com/python/peps/blob/main/peps/pep-0007.rst?plain=1)
