...
changing their status). See PEP Editor Responsibilities & Workflow for
details.
...
can be submitted directly to the Python issue tracker.
...
PEP-able. Posting to the Ideas category of the Python Discourse is usually
the best way to go about this, unless a more specialized venue is appropriate,
such as the Typing category (for static typing ideas)
or Packaging category (for packaging ideas) on the Python Discourse.
...
with write access to the PEP repository are listed for your new file.
...
Developers with write access to the PEP repository may claim PEP
...
PEP editors (see PEP Editor Responsibilities & Workflow). This includes
...
(or a collaborating developer) have write access to the PEP repository.
...
and the draft PEP is committed to the PEP repository,
...
a brief announcement post should be made to the PEPs category
...
opening a Steering Council issue once the authors (and sponsor, if any)
...
PEP-Delegate by notifying the Steering Council
...
the time when when Python was led by a BDFL.
...
(which can be done via a new issue ).
...
PEPs category on the Python Discourse.
...
such as the Language Reference for core features, the Library Reference
for standard library modules or the PyPA Specifications for packaging.
...
- Preamble – RFC 2822 style headers containing meta-data about the
...
- Motivation – The motivation is critical for PEPs that want to
...
e.g. how the feature is supported in other languages.The rationale should provide evidence of consensus within the
...
- Security Implications – If there are security concerns in relation
...
work.This section may include key points and recommended documentation
...
discussions of API details.The final implementation must include test code and documentation
appropriate for either the Python language reference or the
standard library reference.

- Rejected Ideas – Throughout the discussion of a PEP, various ideas
...
rejected idea again in subsequent discussions.In a way this section can be thought of as a breakout section of the
Rationale section that is focused specifically on why certain ideas
were not ultimately pursued.

- Open Issues – While a PEP is in draft, ideas can come up which
...
- Acknowledgements – Useful to thank and acknowledge people who have helped
...
- Footnotes – A collection of footnotes cited in the PEP, and
a place to list non-inline hyperlink targets.

Change History – A summary of major changes the PEP has undergone, based ondiscussions and feedback. Think of this as a “changelog” or “release notes”
...
following along to understand the evolution of your PEP. Here is [an
example](../pep-0694/#change-history).
...
contains instructions and a PEP template.

The PEP text files are automatically
converted to HTML
(via a Sphinx-based build system)
...
See the Topic Index for the existing topics.
...
as a GitHub issue or GitHub pull request.
...
For guidance on modifying other PEPs, consult the PEP Maintenance section.
...
Note that developers with write access to the PEP repository may
...
access to the PEP repository is added to .github/CODEOWNERS.
...
sometimes it’s a special/joke number, like 666 or 3141).Remember that numbers below 100 are meta-PEPs.
...
Post-History header.
 ...
 [Page Source (GitHub)](https://github.com/python/peps/blob/main/peps/pep-0001.rst?plain=1)