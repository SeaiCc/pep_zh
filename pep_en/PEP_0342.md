...
another value as a response to the GeneratorExit exception?I originally chose TypeError because it represents gross misbehavior of
...
http://effbot.org/zone/consumer.htm for the specification.)However, looking more closely at the consumer interface, it seems that the
...
advance to its first yield point when initially called:def consumer(func):
 def wrapper(*args,**kw):
 gen = func(*args, **kw)
 gen.next()
 return gen
...
inability to pass values or exceptions into generators.)import collections
...
closed):# coroutine function that echos data back on a connected
# socket
#
...
- Copyright
 ...
 [Page Source (GitHub)](https://github.com/python/peps/blob/main/peps/pep-0342.rst?plain=1)
