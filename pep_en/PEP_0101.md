...
- A GPG key.Python releases before 3.14 are digitally signed with GPG; for these you’ll
need a key, which hopefully will be on the “web of trust” with at least one of
the other release managers.

Note

GPG instructions in this PEP can be ignored for Python 3.14 and
...
- Administrator access to python/cpython and membership of the Release
Managers team.

- An administrator account on www.python.org, including an “API key”.

- Write access to the python/peps repository.If you’re reading this, you probably already have this–the first
...
python/release-tools. This helps by automating many of the following steps,
and guides you to perform some manual steps.

- Log into Discord and join the Python Core Devs server. Ask another
RM for an invite.You probably need to coordinate with other people around the world.
...
recommended in the Python Developer’s Guide,
...
- Regenerate Lib/pydoc-topics.py.While still in the Doc directory, run:
...
In python/release-tools, go to the build-release
...
signatures, SBOMs, etc.While you’re waiting for the files to finish uploading, you can continue
on with the remaining tasks. You can also ask folks on Discord
and/or discuss.python.org to download the files as they finish uploading
...
which have references to python38:ls PC/pyconfig.h.in PCbuild/rt.bat | xargs sed -i 's/python3\(\.\?\)[0-9]\+/python3\19/g'
...
https://github.com/python/cpython/settings/branchesAlso, add 3.x and needs backport to 3.X labels to the GitHub repo.
...
python/release-tools repo (next to release.py). You run the
...
Write the announcement on discuss.python.org. This is the
...
Update python-releases.toml to reflect the new branches and versions.
...
pushes, etc).
Verify that the online docs are building
properly (this may take up to 24 hours for a complete build on the website).

## What Next?

Verify! Pretend you’re a user: download the files from www.python.org, and
...
In python-releases.toml, set the branch status to end-of-life.
...
Last modified: 2026-03-07 22:09:39 GMT
...
- Copyright
 ...
 [Page Source (GitHub)](https://github.com/python/peps/blob/main/peps/pep-0101.rst?plain=1)
