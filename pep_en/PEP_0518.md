...
its setup() function [3]. This solution has a number
of issues, such as:
...
pyproject.toml that is written in the TOML format [6].

This format was chosen as it is human-usable (unlike JSON [7]),
it is flexible enough (unlike configparser [9]), stems
from a standard (also unlike configparser [9]), and it
is not overly complex (unlike YAML [8]). The TOML format is
already in use by the Rust community as part of their
Cargo package manager [14] and in private email stated they have
...
in the Other file formats section. The authors do realize, though,
...
Schema [15] would match the data format:
...
positive arguments in favour of TOML) can be found at [16].
...
The JSON format [7] was initially considered but quickly
...
The YAML format [8] was designed to be a superset of JSON
[7] while being easier to work with by hand. There are three main
issues with YAML.
...
PyYAML [10] which is a large project of a few thousand lines of
...
configparser [9] accepts was considered. Unfortunately
...
safe if parsed by ast.literal_eval() [13].
...
pysettings.tomlMost reasonable alternative.
pypa.tomlWhile it makes sense to reference the PyPA [[11]](#pypa), it is a
somewhat niche term. It’s better to have the file name make sense
without having domain-specific knowledge.
pybuild.tomlFrom the restrictive perspective of this PEP this filename makes
sense, but if any non-build metadata ever gets added to the file
then the name ceases to make sense.
pip.tomlToo tool-specific.
meta.tomlToo generic; project may want to have its own metadata file.
setup.tomlWhile keeping with traditional thanks to setup.py, it does not
...
pymeta.tomlNot obvious to newcomers to programming and/or Python.
pypackage.toml & pypackaging.tomlName conflation of what a “package” is (project versus namespace).
pydevelop.tomlThe file may contain details not specific to development.
pysource.tomlNot directly related to source code.
pytools.tomlMisleading as the file is (currently) aimed at project management.
dstufft.tomlToo person-specific. ;)
...
- Copyright
 ...
 [Page Source (GitHub)](https://github.com/python/peps/blob/main/peps/pep-0518.rst?plain=1)
