...
allocation of the object.See also gc.get_referrers() and sys.getsizeof() functions.

get_traceback_limit() function:

> Get the maximum number of frames stored in the traceback of a trace.The tracemalloc module must be tracing memory allocations to get
...
allocations, False otherwise.See also start() and stop() functions.
...
or equal to 1.Storing more than 1 frame is only useful to compute statistics
...
PythonCall take_snapshot() function to take a snapshot of traces
...
Return a new Snapshot instance.The snapshot does not include memory blocks allocated before the
...
> Filter on traces of memory blocks.See the fnmatch.fnmatch() function for the syntax of
...
line number lineno.If inclusive is False (exclude), ignore memory blocks
allocated in a file with a name matching filename_pattern at
line number lineno.
...
checked.This attribute is ignored if the traceback limit is less than 2.
...
sorted list of StatisticDiff instances grouped by *group_by*.See the statistics() method for group_by and cumulative
parameters.
...
the traces.All inclusive filters are applied at once, a trace is ignored if no
...
> Statistic on memory allocations.Snapshot.statistics() returns a list of Statistic instances.
...
Snapshot instance.Snapshot.compare_to() returns a list of StatisticDiff
...
> Trace of a memory block.The Snapshot.traces attribute is a sequence of Trace
instances.
...
the oldest frame.A traceback contains at least 1 frame. If the tracemalloc module
...
- PySizer: Google Summer of Code 2005 project by
Nick Smallbone.
...
- gdb-heap: gdb script written in
...
- Copyright
 ...
 [Page Source (GitHub)](https://github.com/python/peps/blob/main/peps/pep-0454.rst?plain=1)
