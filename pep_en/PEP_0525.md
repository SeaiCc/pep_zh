...
- Implement an aclose method on asynchronous generators
...
a StopAsyncIteration occur.This is very similar to what the close() method does to regular
Python generators, except that an event loop is required to execute
aclose().

- Raise a RuntimeError, when an asynchronous generator executes
a yield expression in its finally block (using await
is fine, though):async def gen():
 try:
 yield
 finally:
...
Add two new methods to the sys module:
...
- agen.asend(val): Returns an awaitable, that pushes the
val object in the agen generator. When the agen has
not yet been iterated, val must be None.Example:
...
agen.athrow(typ, [val, [tb]]): Returns an awaitable, that
throws an exception into the agen generator.Example:
...
to the caller.
agen.__name__ and agen.__qualname__: readable and writable
name and qualified name attributes.
agen.ag_await: The object that agen is currently awaiting
on, or None. This is similar to the currently available
gi_yieldfrom for generators and cr_await for coroutines.
agen.ag_frame, agen.ag_running, and agen.ag_code:
...
PyAsyncGenASend: the awaitable object that implements
__anext__ and asend() methods.

- PyAsyncGenAThrow: the awaitable object that implements
...
- When PyAsyncGenASend.send(val) is called for the first time,
val is pushed to the parent agen object (using existing
facilities of PyGenObject.)Subsequent iterations over the PyAsyncGenASend objects, push
None to agen.
...
*exc is thrown into the parent agen object.Subsequent iterations over the PyAsyncGenASend objects, push
None to agen.
...
- types.AsyncGeneratorType – type of asynchronous generator
object.

- sys.set_asyncgen_hooks() and sys.get_asyncgen_hooks()
...
[[1](#id3)]
[https://github.com/1st1/cpython/tree/async_gen](https://github.com/1st1/cpython/tree/async_gen)

[[2](#id1)]
[https://mail.python.org/pipermail/python-dev/2016-September/146267.html](https://mail.python.org/pipermail/python-dev/2016-September/146267.html)

[[3](#id2)]
[http://bugs.python.org/issue28003](http://bugs.python.org/issue28003)
...
- Copyright
 ...
 [Page Source (GitHub)](https://github.com/python/peps/blob/main/peps/pep-0525.rst?plain=1)
